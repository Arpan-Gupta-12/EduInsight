...
def _bfs_peer_mentor_matcher(cur, student_id, failing_subjects):
    ...
    cur.execute("""
        SELECT e.student_id, s.full_name, g.total_marks
        FROM enrollments e
        JOIN students s ON s.student_id = e.student_id
        JOIN grades g   ON g.enrollment_id = e.enrollment_id
        WHERE e.course_id = ? AND g.grade_point >= 7
...
def get_all_students(limit=500):
    """Return ALL students with CGPA, attendance, risk level, SGPA history."""
    conn = get_conn()
    cur  = conn.cursor()
    ...
    for student in base_list:
        ...
        cur.execute("""
            SELECT ROUND(sgpa, 2) FROM v_semester_gpa
            WHERE student_id = ? ORDER BY semester_id
        """, (sid,))
        sgpas = [r[0] for r in cur.fetchall()]

        if len(sgpas) >= 2:
            cgpa_trend = round(sgpas[-1] - sgpas[0], 2)
        else:
            cgpa_trend = 0.0
